...
export default async function scSupport(successCriteria, allIssues, data) {
  ...
  const filteredCriteria = Object.fromEntries(
    Object.entries(successCriteria).filter(
      ([_key, criterion]) =>
        matchesVersion(criterion.versions, targetWcagVersion) &&
  ...
  for (const key in filteredCriteria) {
    ...
    const relevantSevereIssues = relevantIssues.filter(
    ...
    const scLabel =
      relevantIssues.length > 0
        ? `<a href="#${slugify(successCriteria[key].name)}">${key}: ${successCriteria[key].name}</a>`
        : `${key}: ${successCriteria[key].name}`;

    const scReferenceLink = scUri(key, successCriteria);

    output += `<tr><td>${scLabel}</td><td>${successCriteria[key].level}</td><td>${support}</td><td><a href="${scReferenceLink}" target="_blank">
